...
public class VideoView extends JPanel {
	private static final String VideoVO = null;
	// member field
	...
	JTable tableVideo; // 실질 적으로 보여지는 테이블화면

	VideoTableModel tbModelVideo; // 실제 테이블정보
	...
	public void connectDB() { // DB연결
		try {
			...
			System.out.println("비디오 관리드라이버 로딩 실패 : " + e.getMessage());
			e.printStackTrace();
		}
	}
		...
		tableVideo.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent ev) {
				...
				try {
					...
					VideoVO vo = model.sleleteByVnum(vNum);
	...
	public void modifyVideo() {
		// (1) 화면의 사용자 입력값 얻어오기
		int v_number = Integer.parseInt(tfVideoNum.getText());
		String v_type = (String) comVideoJanre.getSelectedItem();
		int count = Integer.parseInt(tfInsertCount.getText());
		String v_name = tfVideoTitle.getText();
		String v_director = tfVideoDirector.getText();
	...
	public void searchVideo() {
		...
		try {
			...
			tbModelVideo.fireTableDataChanged(); // 모델쪽에서 데이터 변경을 뷰쪽으로 신호를 줌
		} catch (Exception ex) {
